module "cluster" {
  ...
  global_cluster_identifier = "rds-cluster-${var.aws_shot_region}-${var.environment}-${var.service_name}"
  create_global_cluster     = each.value.create_global_cluster

  engine = "aurora-mysql"

  engine_version                  = "8.0.mysql_aurora.3.01.0"
  name                            = "rds-${var.aws_shot_region}-${var.environment}-${var.service_name}-${each.value.name}"
  ...
  secet_manager_name = "sm-${var.aws_shot_region}-${var.environment}-${var.service_name}-${each.value.master_username}_pw"
...
resource "aws_rds_cluster_parameter_group" "aurora_cluster_parameter" {
  ...
  parameter {
    ...
    value        = (var.environment == "prd" || var.environment == "nprd") ? 8388608 : 262144
    apply_method = "pending-reboot"
  }
  parameter {
  ...
  parameter {
    ...
    value = (var.environment == "prd" || var.environment == "nprd") ? 2097152 : 786432
  }
  parameter {
    name  = "tmp_table_size"
...
##### User Account #####
module "secret_value" {
  for_each = var.marketplace_mysql_ids
...
##### User Account #####
module "secret_value_launch" {
  for_each = var.launchpad_mysql_ids
...
module "secret_value_system" {
  for_each = var.systemadmin_mysql_ids

resource "aws_cloudfront_distribution" "marketplace_stg" {
  origin {
    domain_name = "s3-ue2-stg-naemo-fe-marketplace.s3.amazonaws.com"
    origin_id   = "s3origin"
    
    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.marketplace_stg.cloudfront_access_identity_path
    }
  }

  origin {
    domain_name = "s3-ue2-stg-naemo-launchpad.s3.amazonaws.com"
  ...
  origin {
    domain_name = "s3-ue2-stg-naemo-collection.s3.amazonaws.com"
  ...
  origin {
    domain_name = "s3-ue2-stg-naemo-member.s3.amazonaws.com"
  ...
  origin {
    domain_name = "s3-ue2-stg-naemo-nft.s3.amazonaws.com"
  ...
  comment             = "Marketplace Frontend(stg)"
 ...
 # Cache behavior with precedence 
  ...
  viewer_certificate {
    ...
    acm_certificate_arn = aws_acm_certificate.cert_virginia_stg.arn
...
resource "aws_cloudfront_origin_access_identity" "marketplace_stg" {
  comment = "marketplace_stg"
}
...
resource "tls_private_key" "coderay_private_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "aws_key_pair" "coderay_keypair" {
  key_name   = var.coderay_keypair_name
  public_key = tls_private_key.coderay_private_key.public_key_openssh
}
